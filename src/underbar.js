...
(function() {
  ...
  _.reject = function(collection, test) {
    ...
    var arr=_.filter(collection,test), narr=[];
    for(var i=0;i< collection.length;i++){
      var temp=collection[i],test=true;
      for(var j=0;j< arr.length;j++){
        if(temp==arr[j])test=false;
      }
      if(test)narr.push(temp);
    }
    return narr;
  };
  ...
  _.uniq = function(array,test,iterator) {
    var arr=[];arr.push(array[0]);
    test= test==undefined ? false : true;
    if (iterator==undefined){
      iterator=function (value){return value+1;};
    }
    for(var i=0;i<array.length;i=iterator(i)){
      var temp=array[i],test=true;
      for(var j=0;j<arr.length;j++){
        if(temp==arr[j])test=false;
      }
      if(test)arr.push(temp);
    }
    return arr;
  };
  ...
  _.some = function(collection, iterator) {
    ...
    //check through every. if it hits false, continue. If it hits true, return.
    if(iterator==undefined){
      iterator=_.identity;
    }
    ...
    var some=_.every(arr, function(value){
      if(iterator(value)==true)return false;
      else return true;
    });
    return !some;
  }
